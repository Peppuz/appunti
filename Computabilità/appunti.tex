...
In questo capitolo introduciamo e definiamo gli automi a pila(Pushdown Automata), usato per definire un linguaggio context-free:
questa tipologia di automa è un estensione del $\epsilon-NFA$, con l'aggiunta dello stack, struttura dati che
può leggere, pushare e poppare solo dalla testa.

\begin{figure}
  \includegraphics{/img/PDA.png}
\end{figure}
La presenza dello stack permette all'automa di ricordare un numero infinito di informazioni, anche se a differenza dei computer normali
l'automa a pila può accedere soltanto alle informazioni presenti in testa allo stack, per questo motivo è in grado soltanto di definire
e riconoscere soltanto i linguaggi context-free.

La definizione formale dell'automa a pila è la seguente:
\begin{defi}
  Si definisce Automa a Pila    $P = (Q, \Sigma, \Delta, \delta, q_0, z_0, F)$ dove si ha:
  \begin{itemize}
...
\end{defi}
  Consideriamo il linguaggio context-free $L_{wwr} = \{ww^R | w \in \{0,1\}^*\}$ definito dalle seguenti regole di produzione:
  $P_g = \{ P \To 0P0 | 1P1 | \epsilon \}$, rappresentabile anche dall'automa $P = (\{q_0,q_1,q_2\},\{0,1\},\{0,1,z_0\},\delta,q_0,z_0,\{q_2\})$.

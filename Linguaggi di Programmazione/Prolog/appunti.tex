...
\begin{itemize}
\item Imperative Languages: linguaggi basati sull'architettura di Von Neumann, utilizzata da tutti i calcolatori moderni,
...
\item Logic Languages: linguaggi in cui un programma è una deduzione logica, ossia ogni istruzione è una formula del linguaggio
      e noi interroghiamo il sistema per sapere se essa fa parte della conoscenza rappresentata nel programma;
...
\item Functional Languages: linguaggi in cui il concetto di funzione è l'unica cosa importante infatti i programmi 
      consistono nell'applicazione di uno o più funzioni agli argomenti.\newline
      Fanno parte di codesto paradigma i linguaggi Lisp, Common Lisp, Scheme, Javascript, ML, Ocaml, Haskell, R, ...
\end{itemize}
Ogni dei seguenti paradigmi può avere la presenza di linguaggi object oriented, come Java, C++, Common Lisp, Python, ...,
infatti il paradigma ad oggetti è ortogonale alla definizione dei seguenti paradigmi, anche se presume alcune caratteristiche tipiche
...
infatti il paradigma imperativo è stato inventato per la computazione ``numerica'' rispetto agli altri paradigmi.
...
\end{itemize}
Il paradigma imperativo ha spopolato fino agli anni '70/80 fino a quando è stato spodestato dal paradigma ad Oggetti e noi lo abbiamo già
affrontato e studiato durante il corso Programmazione 1 e in sto corso vedremo soltanto alcune peculiarità del linguaggio C.
...
calcolatori alcune caratteristiche sono derivate dall'architettura di Von Neumann, ed utilizza uno stile di programmazione descrittivo,
ossia viene definito cosa fa parte e cosa no del problema da rappresentare.
...
Il paradigma funzionale, come visto nel paradigma logico, viene usato per una programmazione simbolica, 
...
Dopo che sono state definite le funzioni necessarie per la computazione, possiamo applicarla su un elemento del dominio
per ottenere un valutazione della funzione e ciò è cosa viene definito per computazione nel paradigma funzionale.\newline
In un linguaggio funzionale puro la valutazione di una funzione viene determinato soltanto dalla funzione e non da i valori in memoria e
questa assenza di effetti causati dalla memoria permette di definire una variabile come una costante matematica ossia il valore non è mutabile
...
\begin{itemize}
\item lo \textit{Stack}  serve per la gestione delle chiamate (soprattutto ricorsive) a procedure attraverso record di attivazione.
\item lo \textit{Heap} serve per la gestione di strutture dati dinamiche (liste, alberi etc...)
\end{itemize}
I linguaggi logici e funzionali (ma anche Java) utilizzano pesantemente lo Heap dato che forniscono come strutture dati default 
...
\begin{defi}
    ...
    \end{itemize}
\end{defi}
Si definisce come \emph{aperto} un termine che non contiene variabili altrimenti si definisce il termine come \emph{chiuso}.\newline
...
una variabile si dice \emph{libera} se non ricade nel campo di azione di un quantificatore altrimenti la variabile si dice \emph{vincolata}.
...
da una asserzione iniziale d’obiettivo e la procedura utilizzata dal Prolog per la computazione è il principio di risoluzione.

Un programma logico è un formato da un insieme di istruzioni, rappresentanti un sottoinsieme di frasi ben formate della logica del primo ordine
e l'ambiente Prolog determina se una data assunzione è verificata o meno nel programma e sotto quali eventuali vincoli avviene.
...
Le regole stabiliscono una relazione di implicazione, indicato con il simbolo \textbf{:-}, infatti definiamo che risulta A se risulta verificato
...
Un programma caricato nel sistema prolog rappresenta una base di conoscenza, rappresentabile nei seguenti modi:
\begin{itemize}
...
e un programma in prolog è una collezione di codeste clausole.\newline
...
se l'unificazione è corretta usiamo "=", ovvero, ricollegandoci al codice sopra:
\begin{minted}{prolog}
?- 42 = 42.
Yes
...
In Prolog la base di conoscenza è nascosta ed è accessibile solo tramite opportuni comandi per cui
...
Yes
\end{minted}
Il predicato \textbf{reconsult} viene usato quando si vuole ricaricare un file (ovvero una base di conoscenza) nell'ambiente Prolog.\newline
...
Il sistema Prolog per dimostrare la veridicità di un goal esegue una sequenza di passi di risoluzione,
...
\begin{itemize}
\item \textbf{regola Left-most}: si sceglie sempre il sottogoal più a sinistra 
\item \textbf{regola Right-most}: si sceglie sempre il sottogoal più a destra
...
\begin{itemize}
\item stack delle scelte: contiene l'insieme delle scelte possibili;ad ogni fase della valutazione contiene i puntatori alle
      scelte aperte nelle fasi precedenti della dimostrazione
\item stack di esecuzionei: contiene i record di attivazione delle varie procedure, ovvero le sostituzioni per l'unificazione delle varie regole.
\end{itemize}
...
\begin{itemize}
\item \textbf{green cut:} utili per esprimere “determinismo” e quindi per rendere più efficiente il programma in quanto non vengono analizzate
                          clausole certamente false.
\item \textbf{red cut:} usati per soli scopi di efficienza, hanno per caratteristica principale quella di omettere alcune condizioni
                        esplicite in un programma e, soprattutto, quella di modificare la semantica del programma equivalente senza cuts.\newline
       Hanno degli effetti indesiderabili ma sono utili per cui vanno maneggiati con cura per evitare di effettuare dei danni.
\end{itemize}
...
Un programma Prolog si dice \textbf{deterministico} quando una sola delle clausole serve (o si vorrebbe servisse) per provare un dato goal.\newline
Si usano quindi i green cuts:
\begin{minted}{prolog}
merge([X | Xs], [Y | Ys], [X | Zs]) :-
	X < Y, !,
	merge(Xs, [Y | Ys], Zs).
...
In $q_1$ posso fermarmi e generare solo la stringa $a$ o produrre altri caratteri in numero variabile. Le stringhe qui prodotte
